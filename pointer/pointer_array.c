...
int main(){
    ...
    int (*pArr_2)[3]; // 声明一个指针，指向一个拥有三个元素的数组
    pArr_2=arr_2; 
    printf("arr_2[0][0]: %d: %p\n",arr_2[0][0], &arr_2[0][0]);
    printf("*(pArr_2+0): %d: %p\n", **(pArr_2+0), *(pArr_2+0));
    printf("arr_2[1][1]: %d: %p\n",arr_2[1][1], &arr_2[1][1]);
    printf("*(pArr_2+1)+1: %d: %p\n", *(*(pArr_2+1)+1), *(pArr_2+1)+1);
    pArr_2++; // 跳过，步长为3，跳过3个元素
    printf("---pArr_2前进一步\n");
    printf("*(pArr_2+0)+0: %d: %p\n", *(*(pArr_2+0)+0), *(pArr_2+0)+0); // 输出跳过之后的第二维的第一个数组中的第一个元素，和其地址
    printf("*(pArr_2+0)+1: %d: %p\n", *(*(pArr_2+0)+1), *(pArr_2+0)+1); // 输出跳过之后的第二维的第二个数组中的第一个元素，和其地址
